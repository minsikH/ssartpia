fetch("../data.json")
  ...
  .then((data) => {
    const rentalExhibitionList = data.rentalExhibitionList;
    const rentalExhibitionListB = data.rentalExhibitionListB;
...
*/
    ...
    function showFilteredResults() {
      ...
      const filteredItems =
        currentType === "all"
          ? rentalExhibitionList
          : rentalExhibitionList.filter((item) => item.type === currentType);
      // 추가적인 검색 조건을 적용
      const results = filteredItems.filter(filterItems);
      // 필터링된 항목들을 결과 리스트에 추가
      ...
      /* 
          // 필터된 항목들을 합쳐서 결과 리스트에 추가
          const combinedItems = [...noticeItems, ...nonNoticeItems]; */
    ...
    const tabLinksC = document.querySelector(".filter-tab.filterOnB");

    const searchInputB = document.querySelector(".data .search_bar_input");

    const resultListB = document.querySelector(
      ".data .result_list_area > tbody"
    );
    /*   const resultNumElement = document.querySelector('.result_num'); */

    let currentTypeB = "all"; // 현재 선택된 타입을 추적
    let currentPageB = 1; // 현재 페이지를 추적
    const itemsPerPageB = 20; // 한 페이지당 보여질 아이템 개수
    const pageButtonsLimitB = 5; // 한 번에 표시될 페이지 버튼의 개수

    // 필터링 함수를 정의
    function filterItemsB(item) {
      const searchText = searchInputB.value.trim().toLowerCase(); // 검색어를 소문자로 변환
      const selectBoxResult = document
        .querySelector(".data .select_box_result")
        .textContent.trim(); // select_box_result의 텍스트를 가져옴
      ...
      const typeFilter = currentTypeB === "all" || item.type === currentTypeB;

      return selectBoxResult && searchText && typeFilter;
    }

    tabLinksC.addEventListener("click", function () {
      resetAndApplyFilterB();
      showFilteredResultsB();
      accordion();
    });

    // 필터링된 결과를 리셋하고 새로운 필터를 적용하는 함수
    function resetAndApplyFilterB() {
      searchInputB.value = ""; // 검색어 입력란 초기화
      document.querySelector(".data .select_box_result").textContent = "전체"; // selectBoxResult 초기화
      showFilteredResultsB(); // 필터링된 결과를 업데이트
    }

    // 통합 검색 버튼을 클릭했을 때 필터링 결과를 업데이트
    document
      .querySelector(".data .search_selectA")
      .addEventListener("click", function () {
        currentPageB = 1; // 검색이 다시 시작되므로 페이지를 초기화
        showFilteredResultsB();
        accordion();
      });
    ...
    function moveToPage(event) {
      event.preventDefault();
      currentPageB = parseInt(event.target.dataset.page);
      renderItemsB(currentPageB);
      renderPaginationB();
    }

    // 페이지네이션을 렌더링하는 함수
    function renderPaginationB() {
      if (resultListB) {
        const totalPage = Math.ceil(
          resultListB.children.length / itemsPerPageB
        );
        const paginationList = document.getElementById("js-paginationB");
        paginationList.innerHTML = "";

        // 페이지 버튼 추가
        const startPage =
          Math.floor((currentPageB - 1) / pageButtonsLimitB) *
            pageButtonsLimitB +
          1;
        const endPage = Math.min(startPage + pageButtonsLimitB - 1, totalPage);
        for (let i = startPage; i <= endPage; i++) {
          const pageButton = document.createElement("li");
        ...
        if (startPage > 1) {
          ...
          prevPageButton.classList.add("paginationBox_prev");
          prevPageButton.innerHTML = '<a href="#" id="prev"><span></span></a>';
          prevPageButton.addEventListener("click", moveToPrevPageB);
          paginationList.prepend(prevPageButton); // 이전 버튼을 페이지 버튼 앞에 추가
        }
        ...
        if (endPage < totalPage) {
          ...
          nextPageButton.classList.add("paginationBox_next");
          nextPageButton.innerHTML = '<a href="#" id="next"><span></span></a>';
          nextPageButton.addEventListener("click", moveToNextPageB);
          paginationList.appendChild(nextPageButton);
        }
        ...
        firstPageButton.classList.add("paginationBox_first");
        firstPageButton.innerHTML = '<a href="#" id="first"><span></span></a>';
        firstPageButton.addEventListener("click", moveToFirstPageB);
        paginationList.prepend(firstPageButton); // 첫 페이지 버튼을 페이지 버튼 앞에 추가
        ...
        lastPageButton.classList.add("paginationBox_last");
        lastPageButton.innerHTML = '<a href="#" id="last"><span></span></a>';
        lastPageButton.addEventListener("click", moveToLastPageB);
        paginationList.appendChild(lastPageButton);
    ...
    // 이전 페이지로 이동하는 함수
    function moveToPrevPageB(event) {
      event.preventDefault();
      const totalPage = Math.ceil(resultListB.children.length / itemsPerPageB);

      // 현재 페이지 그룹의 첫 번째 페이지 계산
      const firstPageOfCurrentGroup = Math.max(
        Math.floor((currentPageB - 1) / pageButtonsLimitB) * pageButtonsLimitB +
          1,
        1
      );
      ...
      // 이전 페이지 그룹의 첫 번째 페이지 계산
      const firstPageOfPrevGroup = Math.max(
        firstPageOfCurrentGroup - pageButtonsLimitB,
        1
      );

      // 이전 페이지 그룹의 첫 번째 페이지로 이동
      currentPageB = firstPageOfPrevGroup;
      renderItemsB(currentPageB);
      renderPaginationB();
    }

    // 다음 페이지로 이동하는 함수
    function moveToNextPageB(event) {
      event.preventDefault();
      const totalPage = Math.ceil(resultListB.children.length / itemsPerPageB);
      const lastPageOfCurrentGroup = Math.min(
        Math.ceil(currentPageB / 5) * 5,
        totalPage
      );
      if (lastPageOfCurrentGroup < totalPage) {
        currentPageB = lastPageOfCurrentGroup + 1;
        renderItemsB(currentPageB);
        renderPaginationB();
      }
    }
    // 첫 페이지로 이동하는 함수
    function moveToFirstPageB(event) {
      event.preventDefault();
      currentPageB = 1;
      renderItemsB(currentPageB);
      renderPaginationB();
    ...
    // 마지막 페이지로 이동하는 함수
    function moveToLastPageB(event) {
      event.preventDefault();
      const totalPage = Math.ceil(resultListB.children.length / itemsPerPageB);
      currentPageB = totalPage;
      renderItemsB(currentPageB);
      renderPaginationB();
    }

    // 페이지네이션 및 아이템 렌더링 함수
    function renderItemsB(pageNumber) {
      if (resultListB) {
        const startIndex = (pageNumber - 1) * itemsPerPageB;
        const endIndex = Math.min(
          startIndex + itemsPerPageB,
          resultListB.children.length
        );

        for (let i = 0; i < resultListB.children.length; i++) {
          resultListB.children[i].style.display =
    ...
    function showFilteredResultsB() {
      resultListB.innerHTML = ``;
      const filteredItems =
        currentTypeB === "all"
          ? rentalExhibitionListB
          : rentalExhibitionListB.filter((item) => item.type === currentTypeB);
      const results = filteredItems.filter(filterItemsB);

      if (results.length === 0) {
        const noResultsMessage = `<tr class="contentAcodian">
                                          <td colspan="4" class="content noContent">등록된 글이 없습니다.</td>
                                      </tr>`;
        resultListB.insertAdjacentHTML("beforeend", noResultsMessage);
      } else {
        const noticeItemsB = results.filter((item) => item.notice === "공지");
        const nonNoticeItemsB = results.filter(
          (item) => item.notice !== "공지"
        );

        noticeItemsB.forEach((item) => {
          const htmlString = `<tr class="contentTitle">
                                              <td data-table="num"><span class="notice">${item.notice}</span></td>
                                              <td data-table="title">${item.title}</td>
                                              <td data-table="writer">${item.writer}</td>
          ...
          resultListB.insertAdjacentHTML("beforeend", htmlString);
        });

        nonNoticeItemsB.forEach((item, index) => {
          const htmlString = `<tr class="contentTitle">
                                      <td data-table="num">${index + 1}</td>
                                      <td data-table="title">${item.title}</td>
                                      <td data-table="writer">${
                                        item.writer
                                      }</td>
                                      <td data-table="date">${item.date}</td>
                                    </tr>
